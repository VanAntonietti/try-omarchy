...
for command in pacstrap arch-chroot cargo curl git gzip python3 mke2fs mount umount repo-add sha256sum zstd; do
...
"$guest_dir/scripts/install-omarchy-link-development.sh" \
  --root "$root" \
  --work "$work"
